...
public class PenetrationTestSpec extends JenkinsPipelineSpecification {
  ...
  def setup() {
    PenetrationTest = loadPipelineScriptForTest("sdp/libraries/owasp_zap/penetration_test.groovy")
  }
  ...
  def "ZAP Scan is Run" () {
    setup:
      ...
      explicitlyMockPipelineVariable("inside_sdp_image")
    when:
      PenetrationTest()
    then:
      1 * getPipelineMock("inside_sdp_image.call")(_) >> { _arguments ->
